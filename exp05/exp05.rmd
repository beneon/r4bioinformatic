...
# 然后，计算同样的over_inhibit，days和Target Name组内，F组的mean减去control组的mean的值
df_cyclin_stat_F_ctrl_joined = df_cyclin_stat_F_ctrl_joined %>% mutate(ddCt_mean = __)
# 这里认为对照组和F组相互独立，当x和y相互独立时var_(x-y)=var_x+var_y, 而sd=var^0.5，依据这一关系计算ddCt的标准差
df_cyclin_stat_F_ctrl_joined = df_cyclin_stat_F_ctrl_joined %>% mutate(ddCt_sd = __)
# 另外，还可以计算2^-ddCt的值（选做）
```
